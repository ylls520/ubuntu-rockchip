...
# 计算根文件系统大小并创建ext4镜像
SIZE=$(du -sm chroot/ | awk '{print $1}')
SIZE=$((SIZE + SIZE/5))  # 增加20%空间
echo "Creating ext4 image of size ${SIZE}MB..."
dd if=/dev/zero of="../ubuntu-${RELASE_VERSION}-preinstalled-${FLAVOR}-arm64.img" bs=1M count=${SIZE}
mkfs.ext4 -F -L "ubuntu-rootfs" "../ubuntu-${RELASE_VERSION}-preinstalled-${FLAVOR}-arm64.img"

# 挂载并复制文件
mkdir -p mnt
mount -o loop "../ubuntu-${RELASE_VERSION}-preinstalled-${FLAVOR}-arm64.img" mnt
cp -a chroot/* mnt/
sync && umount mnt
rm -rf mnt
